...
class MlxNormalizerAdapter(Normalizer):
    ...
    async def normalize_dose(self, field: ExtractedField) -> NormalizedDose | None:
        ...
        try:
            ...
            logger.warning("Fallo al llamar al normalizador MLX; se retorna None")
            return None
    ...
    async def _call_model(self, value: str) -> str:
        response = await self._client.chat.completions.create(
            model=self._model,
            messages=[{"role": "user", "content": NORMALIZATION_PROMPT.format(value=value)}],
            temperature=0.0,
        )
